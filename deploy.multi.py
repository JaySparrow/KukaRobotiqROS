    ...
    try:
        ...
        for curr_repeat in range(num_repeats):
            ...
            while not is_done:
                ...
                #     # if env.progress - search_init_steps > 40 and env.progress + retry_steps < cfg["max_episode_length"]: # manual
                #         print(f"    > Retrying at {env.progress}...")
                #         # env.lift_wrench_head(num_steps=int(retry_steps*0.25))
                #         # env.move_gripper_to_init_wrench_pose(timeout=retry_steps*0.75)
                #         # for _ in range(5):
                #         #     env.move_gripper_to_init_wrench_pose(timeout=retry_steps/5)
                #         env.move_gripper_to_init_wrench_pose(timeout=retry_steps)
        ...
        print(f"# Retry Eps / {num_repeats}:", np.sum(np.array(num_retry) > 0))
        print(f"Avg Extra Steps due to Retry / Episode", np.sum(num_retry_step) / num_repeats)